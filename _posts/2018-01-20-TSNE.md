...
在现如今的大数据时代，数据量相对于过去急剧膨胀，数据维度也变得越来越多，数据的总的来说将会变得越来越复杂。比如高清晰度的图片，其中的数据维度指的是像素的数量级，范围从数千到数百万。计算机可以处理任意多维的数据集，但我们人类的认知暂时只局限于三维空间，计算机得出的结果仍然需要我们来复核和解释，所以需要通过一些方法有效地可视化高维度数据。  
